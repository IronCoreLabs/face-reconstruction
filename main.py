...
class FaceReconstruction:
    ...
    async def perform_face_reconstruction(
        self,
        target_emb,
        pregen=True,
        encrypt=False,
        ...
        Arguments:
            ...
            encrypt: Boolean
                Whether to encrypt the embedding or not before attempting the reconstruction.
            pregen_offset: int
        ...
        if pregen:
            ...
            best_latent, best_embedding = self.find_closest_pregen(
                target_emb,
                self.pregen_latents,
                self.pregen_embeddings,
                offset=pregen_offset,
            )
        ...
        with torch.no_grad():
            best_face = self.postprocess_stylegan_faces(
                self.infer_stylegan_faces(current_latent)
            )[0].cpu()
        ...
        with torch.no_grad():
            for i in range(iters):
                ...
                neighbor_latents = (
                    current_latent + std * torch.randn([16, self.G.z_dim]).cuda()
                )
...
async def main():
    ...
    parser.add_argument(
        "--anneal",
        action="store_true",
        help="Use simulated annealing in the reconstruction algorithm (by default, will use greedy algorithm.",
    )
    ...
    parser.add_argument(
        "--save_details",
        action="store_true",
        help="Save .pt file with detailed results",
    )
    parser.add_argument(
        "--encrypt",
        action="store_true",
        help="Encrypt the embedding before reconstructing. Saves to output_encrypted.png",
    )
